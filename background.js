...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  if (changeInfo.status === 'complete' && tab.url && tab.url.includes('dev.azure.com')) {
    chrome.tabs.sendMessage(tabId, { type: "RUN_DETECTION" }, response => {
      // Check chrome.runtime.lastError to prevent an error from appearing in the console
      // if the content script isn't on the page.
      if (chrome.runtime.lastError) {
        // This is expected on pages that are not work item pages.
      }
    });
  }
});